export const emailTranslations = {
  // Greek (Ελληνικά) - Default Language
  el: {
    // Booking Confirmation Email
    booking_confirmation_subject: 'Επιβεβαίωση Κράτησης - Havana Van Booking',
    booking_confirmation_title: 'Επιβεβαίωση Κράτησης',
    booking_confirmation_greeting: 'Αγαπητέ/ή {firstName} {lastName},',
    booking_confirmation_message: 'Η κράτησή σας έχει επιβεβαιωθεί με επιτυχία!',
    booking_confirmation_details: 'Λεπτομέρειες Κράτησης',
    booking_confirmation_number: 'Αριθμός Κράτησης',
    booking_confirmation_date: 'Ημερομηνία Κράτησης',
    booking_confirmation_cart: 'Επιλεγμένο Κάρτ',
    booking_confirmation_location: 'Τοποθεσία',
    booking_confirmation_event_type: 'Τύπος Εκδήλωσης',
    booking_confirmation_guests: 'Αριθμός Επισκεπτών',
    booking_confirmation_total_amount: 'Συνολικό Ποσό',
    booking_confirmation_payment_method: 'Μέθοδος Πληρωμής',
    booking_confirmation_payment_status: 'Κατάσταση Πληρωμής',
    booking_confirmation_special_notes: 'Ειδικές Σημειώσεις',
    booking_confirmation_contact_info: 'Στοιχεία Επικοινωνίας',
    booking_confirmation_email: 'Email',
    booking_confirmation_phone: 'Τηλέφωνο',
    booking_confirmation_address: 'Διεύθυνση',
    booking_confirmation_next_steps: 'Επόμενα Βήματα',
    booking_confirmation_paypal_note: 'Η πληρωμή σας έχει επεξεργαστεί με επιτυχία μέσω PayPal.',
    booking_confirmation_bank_note: 'Παρακαλώ στείλτε το αποδεικτικό της τραπεζικής μεταφοράς για να επιβεβαιώσουμε την πληρωμή σας.',
    booking_confirmation_reservation_note: 'Η κράτησή σας είναι επιβεβαιωμένη. Θα επικοινωνήσουμε μαζί σας σύντομα για την πληρωμή.',
    booking_confirmation_footer: 'Ευχαριστούμε που επιλέξατε το Havana Van Booking!',
    booking_confirmation_contact_support: 'Αν έχετε ερωτήσεις, μη διστάσετε να επικοινωνήσετε μαζί μας.',
    
    // Admin Notification Email
    admin_notification_subject: 'Νέα Κράτηση - Havana Van Booking',
    admin_notification_title: 'Νέα Κράτηση',
    admin_notification_message: 'Έχει δημιουργηθεί μια νέα κράτηση που απαιτεί την προσοχή σας.',
    admin_notification_customer_info: 'Στοιχεία Πελάτη',
    admin_notification_booking_summary: 'Περίληψη Κράτησης',
    admin_notification_view_booking: 'Προβολή Κράτησης',
    admin_notification_admin_panel: 'Πάνελ Διαχείρισης',
    
    // Booking Status Update Email
    status_update_subject: 'Ενημέρωση Κράτησης - Havana Van Booking',
    status_update_title: 'Ενημέρωση Κράτησης',
    status_update_greeting: 'Αγαπητέ/ή {firstName} {lastName},',
    status_update_message: 'Η κατάσταση της κράτησής σας έχει ενημερωθεί.',
    status_update_new_status: 'Νέα Κατάσταση',
    status_update_booking_number: 'Αριθμός Κράτησης',
    status_confirmed: 'Επιβεβαιωμένη',
    status_pending: 'Εκκρεμής',
    status_cancelled: 'Ακυρωμένη',
    status_completed: 'Ολοκληρωμένη',
    status_paid: 'Πληρωμένη',
    status_failed: 'Αποτυχημένη',
    
    // Payment Confirmation Email
    payment_confirmation_subject: 'Επιβεβαίωση Πληρωμής - Havana Van Booking',
    payment_confirmation_title: 'Επιβεβαίωση Πληρωμής',
    payment_confirmation_message: 'Η πληρωμή σας έχει επιβεβαιωθεί με επιτυχία!',
    payment_confirmation_amount: 'Ποσό Πληρωμής',
    payment_confirmation_method: 'Μέθοδος Πληρωμής',
    payment_confirmation_transaction_id: 'Αριθμός Συναλλαγής',
    
    // Common
    currency_symbol: '€',
    date_format: 'DD/MM/YYYY',
    time_format: 'HH:mm',
    email_footer: 'Αυτό το email στάλθηκε αυτόματα από το σύστημα κρατήσεων Havana Van Booking.',
    unsubscribe_text: 'Αν δεν θέλετε να λαμβάνετε αυτά τα emails, παρακαλώ επικοινωνήστε μαζί μας.',
    
    // Booking Details
    booking_dates: 'Ημερομηνίες Κράτησης',
    booking_time: 'Ώρα',
    booking_duration: 'Διάρκεια',
    booking_cost: 'Κόστος',
    selected_items: 'Επιλεγμένα Είδη',
    selected_services: 'Επιλεγμένες Υπηρεσίες',
    shipping_cost: 'Κόστος Αποστολής',
    discount_applied: 'Εφαρμοσμένη Έκπτωση',
    coupon_code: 'Κωδικός Κουπονιού',
    
    // Payment Methods
    payment_paypal: 'PayPal',
    payment_bank_transfer: 'Τραπεζική Μεταφορά',
    payment_cash: 'Μετρητά',
    payment_reservation: 'Κράτηση',
    
    // Status Messages
    status_pending_payment: 'Εκκρεμής Πληρωμή',
    status_payment_verified: 'Πληρωμή Επιβεβαιωμένη',
    status_ready_for_event: 'Έτοιμο για Εκδήλωση',
    
    // Admin-Created Booking Emails
    action_required: 'Απαιτείται Ενέργεια',
    admin_created_booking_message: 'Μια κράτηση δημιουργήθηκε για εσάς. Παρακαλώ εξετάστε τις λεπτομέρειες και προβείτε σε ενέργεια.',
    confirm_booking_button: 'Επιβεβαίωση Κράτησης',
    cancel_booking_button: 'Ακύρωση Κράτησης',
    upload_payment_proof_button: 'Αποστολή Αποδεικτικού Πληρωμής',
    pay_with_paypal_button: 'Πληρωμή με PayPal',
    
    // Bank Transfer Specific
    bank_transfer_instructions_title: 'Στοιχεία Τραπεζικής Μεταφοράς',
    bank_transfer_instructions_message: 'Παρακαλώ μεταφέρετε το ποσό στον παρακάτω λογαριασμό και στείλτε το αποδεικτικό πληρωμής.',
    payment_deadline: 'Προθεσμία Πληρωμής',
    bank_name: 'Όνομα Τράπεζας',
    account_holder: 'Δικαιούχος Λογαριασμού',
    iban: 'IBAN',
    swift_code: 'Κωδικός SWIFT',
    instructions: 'Οδηγίες',
    
    // Confirmation Messages
    booking_confirmed_by_customer: 'Ο πελάτης επιβεβαίωσε την κράτηση',
    booking_cancelled_by_customer: 'Ο πελάτης ακύρωσε την κράτηση',
    payment_proof_submitted: 'Αποδεικτικό πληρωμής υποβλήθηκε',
    action_completed_successfully: 'Η ενέργεια ολοκληρώθηκε με επιτυχία',
    
    // Booking Dates Updated
    booking_dates_updated_title: 'Οι Ημερομηνίες Κράτησης Ενημερώθηκαν',
    booking_dates_updated_message: 'Οι ημερομηνίες της κράτησής σας ενημερώθηκαν από την ομάδα μας.',
    previous_dates: 'Προηγούμενες Ημερομηνίες',
    new_dates: 'Νέες Ημερομηνίες',
    updated_total_amount: 'Ενημερωμένο Συνολικό Ποσό',
    dates_change_notice: 'Παρακαλώ ελέγξτε τις ενημερωμένες ημερομηνίες της κράτησής σας.',

    // Booking Updated (Full Edit)
    booking_updated_title: 'Η Κράτησή Σας Ενημερώθηκε',
    booking_updated_message: 'Η κράτησή σας ενημερώθηκε από την ομάδα μας. Παρακάτω είναι οι αλλαγές:',
    changes_made: 'Αλλαγές που Πραγματοποιήθηκαν',
    cart_changed: 'Το Όχημα Άλλαξε',
    dates_changed: 'Οι Ημερομηνίες Άλλαξαν',
    customer_info_changed: 'Οι Πληροφορίες Επικοινωνίας Ενημερώθηκαν',
    items_changed: 'Τα Προϊόντα Φαγητού Ενημερώθηκαν',
    services_changed: 'Οι Υπηρεσίες Ενημερώθηκαν',
    previous_value: 'Προηγούμενο',
    new_value: 'Νέο',
    items_added: 'Προστέθηκαν',
    items_removed: 'Αφαιρέθηκαν',
    quantities_changed: 'Οι Ποσότητες Άλλαξαν',
    services_added: 'Προστέθηκαν Υπηρεσίες',
    services_removed: 'Αφαιρέθηκαν Υπηρεσίες',
    total_amount_changed: 'Το Συνολικό Ποσό Άλλαξε',
    no_changes: 'Δεν Πραγματοποιήθηκαν Αλλαγές',
    field_email: 'Email',
    field_phone: 'Τηλέφωνο',
    field_address: 'Διεύθυνση',
    field_city: 'Πόλη',
    field_event_type: 'Τύπος Εκδήλωσης',
    field_guests: 'Αριθμός Επισκεπτών',
    payment_method_changed: 'Μέθοδος Πληρωμής Άλλαξε',
    payment_method_update_notice: 'Παρακαλώ ολοκληρώστε την πληρωμή χρησιμοποιώντας το παρακάτω κουμπί',
    pay_now_button: 'Πληρώστε Τώρα',
  },

  // English
  en: {
    // Booking Confirmation Email
    booking_confirmation_subject: 'Booking Confirmation - Havana Van Booking',
    booking_confirmation_title: 'Booking Confirmation',
    booking_confirmation_greeting: 'Dear {firstName} {lastName},',
    booking_confirmation_message: 'Your booking has been successfully confirmed!',
    booking_confirmation_details: 'Booking Details',
    booking_confirmation_number: 'Booking Number',
    booking_confirmation_date: 'Booking Date',
    booking_confirmation_cart: 'Selected Cart',
    booking_confirmation_location: 'Location',
    booking_confirmation_event_type: 'Event Type',
    booking_confirmation_guests: 'Number of Guests',
    booking_confirmation_total_amount: 'Total Amount',
    booking_confirmation_payment_method: 'Payment Method',
    booking_confirmation_payment_status: 'Payment Status',
    booking_confirmation_special_notes: 'Special Notes',
    booking_confirmation_contact_info: 'Contact Information',
    booking_confirmation_email: 'Email',
    booking_confirmation_phone: 'Phone',
    booking_confirmation_address: 'Address',
    booking_confirmation_next_steps: 'Next Steps',
    booking_confirmation_paypal_note: 'Your payment has been successfully processed through PayPal.',
    booking_confirmation_bank_note: 'Please send the bank transfer receipt to confirm your payment.',
    booking_confirmation_reservation_note: 'Your booking is confirmed. We will contact you shortly regarding payment.',
    booking_confirmation_footer: 'Thank you for choosing Havana Van Booking!',
    booking_confirmation_contact_support: 'If you have any questions, please don\'t hesitate to contact us.',
    
    // Admin Notification Email
    admin_notification_subject: 'New Booking - Havana Van Booking',
    admin_notification_title: 'New Booking',
    admin_notification_message: 'A new booking has been created that requires your attention.',
    admin_notification_customer_info: 'Customer Information',
    admin_notification_booking_summary: 'Booking Summary',
    admin_notification_view_booking: 'View Booking',
    admin_notification_admin_panel: 'Admin Panel',
    
    // Booking Status Update Email
    status_update_subject: 'Booking Update - Havana Van Booking',
    status_update_title: 'Booking Update',
    status_update_greeting: 'Dear {firstName} {lastName},',
    status_update_message: 'Your booking status has been updated.',
    status_update_new_status: 'New Status',
    status_update_booking_number: 'Booking Number',
    status_confirmed: 'Confirmed',
    status_pending: 'Pending',
    status_cancelled: 'Cancelled',
    status_completed: 'Completed',
    status_paid: 'Paid',
    status_failed: 'Failed',
    
    // Payment Confirmation Email
    payment_confirmation_subject: 'Payment Confirmation - Havana Van Booking',
    payment_confirmation_title: 'Payment Confirmation',
    payment_confirmation_message: 'Your payment has been successfully confirmed!',
    payment_confirmation_amount: 'Payment Amount',
    payment_confirmation_method: 'Payment Method',
    payment_confirmation_transaction_id: 'Transaction ID',
    
    // Common
    currency_symbol: '€',
    date_format: 'MM/DD/YYYY',
    time_format: 'HH:mm',
    email_footer: 'This email was sent automatically from the Havana Van Booking booking system.',
    unsubscribe_text: 'If you do not wish to receive these emails, please contact us.',
    
    // Booking Details
    booking_dates: 'Booking Dates',
    booking_time: 'Time',
    booking_duration: 'Duration',
    booking_cost: 'Cost',
    selected_items: 'Selected Items',
    selected_services: 'Selected Services',
    shipping_cost: 'Shipping Cost',
    discount_applied: 'Discount Applied',
    coupon_code: 'Coupon Code',
    
    // Payment Methods
    payment_paypal: 'PayPal',
    payment_bank_transfer: 'Bank Transfer',
    payment_cash: 'Cash',
    payment_reservation: 'Reservation',
    
    // Status Messages
    status_pending_payment: 'Pending Payment',
    status_payment_verified: 'Payment Verified',
    status_ready_for_event: 'Ready for Event',
    
    // Admin-Created Booking Emails
    action_required: 'Action Required',
    admin_created_booking_message: 'A booking has been created for you. Please review the details and take action.',
    confirm_booking_button: 'Confirm Booking',
    cancel_booking_button: 'Cancel Booking',
    upload_payment_proof_button: 'Upload Payment Proof',
    pay_with_paypal_button: 'Pay with PayPal',
    
    // Bank Transfer Specific
    bank_transfer_instructions_title: 'Bank Transfer Details',
    bank_transfer_instructions_message: 'Please transfer the amount to the following account and submit your payment proof.',
    payment_deadline: 'Payment Deadline',
    bank_name: 'Bank Name',
    account_holder: 'Account Holder',
    iban: 'IBAN',
    swift_code: 'SWIFT Code',
    instructions: 'Instructions',
    
    // Confirmation Messages
    booking_confirmed_by_customer: 'Customer confirmed the booking',
    booking_cancelled_by_customer: 'Customer cancelled the booking',
    payment_proof_submitted: 'Payment proof submitted',
    action_completed_successfully: 'Action completed successfully',
    
    // Booking Dates Updated
    booking_dates_updated_title: 'Booking Dates Updated',
    booking_dates_updated_message: 'Your booking dates have been updated by our team.',
    previous_dates: 'Previous Dates',
    new_dates: 'New Dates',
    updated_total_amount: 'Updated Total Amount',
    dates_change_notice: 'Please review the updated dates for your booking.',

    // Booking Updated (Full Edit)
    booking_updated_title: 'Your Booking Has Been Updated',
    booking_updated_message: 'Your booking has been updated by our team. Below are the changes:',
    changes_made: 'Changes Made',
    cart_changed: 'Cart Changed',
    dates_changed: 'Dates Changed',
    customer_info_changed: 'Contact Information Updated',
    items_changed: 'Food Items Updated',
    services_changed: 'Services Updated',
    previous_value: 'Previous',
    new_value: 'New',
    items_added: 'Added',
    items_removed: 'Removed',
    quantities_changed: 'Quantities Changed',
    services_added: 'Services Added',
    services_removed: 'Services Removed',
    total_amount_changed: 'Total Amount Changed',
    no_changes: 'No Changes Made',
    field_email: 'Email',
    field_phone: 'Phone',
    field_address: 'Address',
    field_city: 'City',
    field_event_type: 'Event Type',
    field_guests: 'Number of Guests',
    payment_method_changed: 'Payment Method Changed',
    payment_method_update_notice: 'Please complete payment using the button below',
    pay_now_button: 'Pay Now',
  }
}

export type EmailTranslationKey = keyof typeof emailTranslations.el
export type EmailLanguage = keyof typeof emailTranslations

export const getEmailTranslation = (key: EmailTranslationKey, language: EmailLanguage = 'el'): string => {
  return emailTranslations[language][key] || emailTranslations.en[key] || key
}

export const formatEmailText = (text: string, variables: Record<string, string | number>): string => {
  return text.replace(/\{(\w+)\}/g, (match, key) => {
    return variables[key]?.toString() || match
  })
}
